...
# CSS to customize the app appearance with the new color scheme
st.markdown("""
<style>
...
try:
    ...
    # Set matplotlib dark theme to match app
    plt.style.use('dark_background')
    # Customize matplotlib colors to match our scheme
    plt.rcParams['axes.facecolor'] = '#1C1C1E'
    plt.rcParams['figure.facecolor'] = '#1C1C1E'
...
try:
    ...
    # Define a custom plotly theme that matches our color scheme
    orange_theme = dict(
        layout=dict(
            plot_bgcolor='#1C1C1E',
            ...
            colorway=['#FF5733', '#FF8C61', '#FFC107', '#4CAF50', '#2196F3', '#9C27B0', '#E91E63']
        )
    )
except ImportError:
    pass
        ...
        try:
            if 'px' in globals():  # Check if plotly express is available
                fig = px.pie(
                    ...
                    names='Category',
                    hole=0.4,
                    color_discrete_sequence=['#FF5733', '#FF8C61', '#4CAF50', '#FFC107', '#2196F3']
                )
                fig.update_layout(
                    **orange_theme['layout'],
            ...
            with col2:
                ...
                try:
                    if 'px' in globals():
                        ...
                        fig.update_traces(line=dict(color='#FF5733'), marker=dict(color='#FF8C61'))
                        st.plotly_chart(fig, use_container_width=True)
                    else:
    ...
    try:
        if 'px' in globals():
            ...
            fig = px.scatter(
                ...
                color_discrete_sequence=['#FF5733', '#FF8C61', '#FFC107', '#4CAF50', '#2196F3']
            )
            
            # Add quadrant lines
            fig.add_shape(
                type="line", line=dict(dash="dash", color="#F2F2F2"),
            ...
            fig.add_annotation(
                x=avg_usage / 2, y=avg_cost / 2,
                text="Low Cost, Low Usage",
                showarrow=False,
                font=dict(size=10, color="#F2F2F2")
            )
            fig.add_annotation(
                x=avg_usage * 1.5, y=avg_cost / 2,
                text="Low Cost, High Usage (Good Value)",
                showarrow=False,
                font=dict(size=10, color="#F2F2F2")
            )
            fig.add_annotation(
                x=avg_usage / 2, y=avg_cost * 1.5,
                text="High Cost, Low Usage (Poor Value)",
                showarrow=False,
                font=dict(size=10, color="#F2F2F2")
            )
            fig.add_annotation(
                x=avg_usage * 1.5, y=avg_cost * 1.5,
                text="High Cost, High Usage",
                showarrow=False,
                font=dict(size=10, color="#F2F2F2")
            )
...
def render_usage_trends():
    ...
    with col2:
        ...
        try:
            if slope is not None and 'go' in globals():
                ...
                fig.add_trace(go.Scatter(
                    ...
                    line=dict(color='#FF8C61', dash='dash'),
                    marker=dict(size=8, color='#FF8C61')
                ))
    ...
    try:
        if 'px' in globals():
            ...
            fig = px.line(
                ...
                color_discrete_sequence=['#FF5733', '#FF8C61', '#FFC107', '#4CAF50', '#2196F3', '#9C27B0', '#E91E63']
            )
    ...
    try:
        if 'px' in globals():
            ...
            # Add threshold lines
            fig.add_shape(
                type="line", line=dict(dash="dash", color="#F44336", width=2),
                x0=weeks[0], y0=1, x1=weeks[-1], y1=1,
                name="Low Usage Threshold"
            )
            
            fig.add_shape(
                type="line", line=dict(dash="dash", color="#FF8C61", width=2),
                x0=weeks[0], y0=3, x1=weeks[-1], y1=3,
                name="Medium Usage Threshold"
            )
...
def render_calendar():
    ...
    for week in range(6):  # Maximum 6 weeks in a month
        ...
        for i in range(7):
            if (week == 0 and i < first_day) or day_counter > days_in_month:
                ...
                day_style = "background-color: #FF5733; padding: 5px; border-radius: 5px; color: white;" if is_today else ""
                ...
                if day_counter in billing_days:
                    for service in billing_days[day_counter]:
                        cols[i].markdown(
                            f"<div style='background-color: #2C2C2C; padding: 5px; margin-top: 2px; border-radius: 3px; border-left: 3px solid #FF8C61;'>"
    ...
    if upcoming_bills:
        ...
        for bill in upcoming_bills:
            ...
            days_style = "color: #FF5733;" if bill['days_left'] <= 3 else "color: #FF8C61;" if bill['days_left'] <= 7 else ""
